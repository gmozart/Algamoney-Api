...
//Handler, para capturar excessões de respsota das entidades	
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		String mensagemUsuario = messageSource.getMessage("mensagem.invalida", null, LocaleContextHolder.getLocale()); // vai a mensagem
		...
		return handleExceptionInternal(ex, erros , headers,
				HttpStatus.BAD_REQUEST, request); // mensagem tratada, mesmo que só retorne um, vai retornar uma lista.
	}
	...
	public List<Erro> ListaDeErros(BindingResult bindingResult) { //BindingResult tem a lista de todos os erros
		...
		for(FieldError fieldError : bindingResult.getFieldErrors()) { //Field = campos, logo ele pega todos os erros que aconteceram nos atributos
